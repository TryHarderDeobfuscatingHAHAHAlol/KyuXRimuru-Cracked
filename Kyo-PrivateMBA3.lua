...
-- Fails safe: if the blacklist fails to fetch, it won't lock out all players, but logs a warning.
if not fetchSuccess then
    warn(fetchError or "Blacklist fetch failed. Proceeding without blacklist enforcement.")
end
...
local function isBlacklisted()
    ...
    if blacklist[playerName] == true then
        return true
    end
...
if isBlacklisted() then
    LocalPlayer:Kick("BLACKLISTED KA SA PUBLIC SCRIPT KO TANGINAMO!")
    return
end

-- Runtime spoof and blacklist detection
task.spawn(function()
    while task.wait(3) do
        ...
        if isBlacklisted() then
            LocalPlayer:Kick("Blacklisted (runtime).")
            break
        end
    end
end)
